  ...
  Widget _textContent(BuildContext context, ColorScheme cs) {
    return Padding(
      ...
      child: Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          // Text, а не SelectableText: последний заводит собственный
          // selection-делегат и распознаватели жестов на КАЖДЫЙ экземпляр.
          // В длинной ленте текстовых сообщений это заметно утяжеляет
          // построение/лэйаут каждой строки, которую ListView.builder
          // создаёт и уничтожает при быстрой прокрутке.
          //
          // Оборачивать ленту в SelectionArea вместо этого не стал: его
          // собственные распознаватели долгого нажатия конкурируют с уже
          // существующими onLongPressStart здесь и в multi-select
          // протяжке в home_page.dart — реальный риск сломать открытие
          // контекстного меню сообщения. Копирование всего текста
          // сообщения по-прежнему доступно через кнопку-иконку ниже и
          // через долгое нажатие → «Копировать»; теряется только
          // посимвольное выделение части текста.
          Text(
            item.text ?? '',
